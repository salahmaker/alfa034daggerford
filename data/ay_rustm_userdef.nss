...
void main()
{
  ...
  if (oTarget != OBJECT_INVALID && !GetIsObjectValid(oItem) && !GetIsFriend(oTarget)) // Search for walking food if there's nothing lying around
  { if (EquipMetal(oTarget) || CarriesMetal(oTarget) && GetIsEnemy(oTarget, OBJECT_SELF)) {AttackTarget(oTarget);}
