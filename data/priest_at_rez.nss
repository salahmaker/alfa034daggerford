...
void main()
{
  ...
  int iCost = Priest_ComputeCost( 7, Priest_IsAlly( oPCCarry ) );
  ...
  // Add effects
  ActionCastFakeSpellAtObject( SPELL_RESURRECTION, oPCCarry );
  ...
  {
    DelayCommand( 11.0, ALFA_MORGUE_RestorePC( oPCDead, TRUE ) );
    DelayCommand( 11.0, AssignCommand( oPCDead, JumpToLocation( GetLocation( oPCCarry ) ) ) );
    DestroyObject( oCorpseToken );
  }
  else
  {
    ALFA_MORGUE_RestorePC( oPCDead, TRUE );
