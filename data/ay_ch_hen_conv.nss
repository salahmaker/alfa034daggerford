    ...
    else
    {
        ...
        if (GetIsObjectValid(oMaster))
        {
            ...
            if (GetIsObjectValid(oShouter) && oMaster == oShouter && AbleToTalk(OBJECT_SELF)) {
                SetCommandable(TRUE);
                if(nMatch == ASSOCIATE_COMMAND_FOLLOWMASTER && !GetIsInCombat() && GetDistanceToObject(oMaster) > 5.0){
                //JumpToObject(oMaster);
                AssignCommand(OBJECT_SELF, ClearAllActions());
                AssignCommand(OBJECT_SELF, ActionJumpToLocation(GetLocation(oMaster)));
                //SpeakString("Jumping to master");
                }
