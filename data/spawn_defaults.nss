...
//
// History:
//    09/21/03   Cereborn          Created From the old spawn_cfg_global
//                                 The intention here is to seperate out NESS core
//                                 code from user changes.  The notes from previous
//                                 versions have been left intact
//
//    12/31/02   Cereborn          Added defaults for loot table (LT) subflags
//    01/01/03   Cereborn          Added default for diminishing returns
//                                 suppression flag (SX).  Default is 0 (not
//                                 suppressed).
//    01/11/03   Cereborn          Added defaults for new I subflag of SU.  By
//                                 default, individual child processing is false,
//                                 and retry count is 0.
//    01/22/03   Cereborn          Bug fix: initialize module spawn count to
//                                 0, as it may have a non-zero value when a
//                                 module is loaded from a saved-game.
//    02/15/03   Cereborn          Removed many unneeded/unused 'defaults'
//                                 Added default for SP offset
//    08/25/03   Cereborn          Added global flag for default dim returns
//                                 suppression behavior.  This lets the user set
//                                 whether dim returns are suppressed by default
//                                 or not when no SX flag is present.
//                                 Added default value for SX flag (now takes a
//                                 0 or 1)
//    09/21/03   Cereborn          Created From the old spawn_cfg_global
//                                 Added encounter level (EL) default value
//
