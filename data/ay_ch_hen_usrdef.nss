...
void main()
{
    ...
    if (nEvent == 1001)      //onheartbeat
    {
      if(IntToFloat(GetCurrentHitPoints()) / IntToFloat(GetMaxHitPoints()) < 0.33 && d20() == 20){
        ExecuteScript("ay_conggondeath", OBJECT_SELF);   //remove control stone from master
        RemoveHenchman(GetMaster());
        SetLocalInt(OBJECT_SELF, "crazy", 1);
        ActionAttack(GetNearestObject(OBJECT_TYPE_CREATURE));
        SpeakString("*Shakes uncontrollably.*");
      }
      if(GetLocalInt(OBJECT_SELF, "crazy") == 1) ActionAttack(GetNearestObject(OBJECT_TYPE_CREATURE));
      if(GetCurrentHitPoints() == GetMaxHitPoints() && GetLocalInt(OBJECT_SELF, "crazy") == 1){
        //ExecuteScript("ay_conggondeath", OBJECT_SELF);   //remove control stone from master
        SetLocalInt(OBJECT_SELF, "crazy", 0);
        CreateItemOnObject(GetLocalString(OBJECT_SELF, "consname"), GetNearestPC());
        //AddHenchman(GetNearestPC());
        SpeakString("*Calms and returns to normal.*");
