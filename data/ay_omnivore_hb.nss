...
void main()
{
    ...
    //if(GetSpawnInCondition(NW_FLAG_BEHAVIOR_OMNIVORE) ){
      object oCreature = GetNearestCreature(CREATURE_TYPE_IS_ALIVE, TRUE);
      if(oCreature != OBJECT_INVALID){

        if(GetDistanceToObject(oCreature) > 0.1  && GetDistanceToObject(oCreature) < 5.0 && !GetIsFriend(oCreature)){
          if(d2() != 1){
            ActionPlayAnimation(ANIMATION_FIREFORGET_TAUNT);
            ActionSpeakString("RAAAAWWWRRRRRR!!");
          }
          else{
          ChangeToStandardFaction (OBJECT_SELF, STANDARD_FACTION_HOSTILE);
          SetIsTemporaryEnemy(OBJECT_SELF, oCreature);
          SetIsTemporaryEnemy(oCreature, OBJECT_SELF);
