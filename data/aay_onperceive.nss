...
void main()
{
    ...
    object oSpawn = GetNearestObjectByTag("sw_slimypatrol");
      ...
      !GetIgnore(oPerceived))
    {
        ...
        if(GetIsEnemy(oPerceived) && !GetFactionEqual(oPerceived))
        {
            ...
            else //if(bSeen || bHeard)
            {
                ...
                //Turn off patrol for goblins
                if(oSpawn != OBJECT_INVALID){
                  if(GetLocalInt(oSpawn, "f_PatrolRoute") != -1){
                    SetLocalInt(oSpawn, "f_PatrolRoute", -1);
                    SpeakString("Turning off patrol", TALKVOLUME_SHOUT);
                    ClearAllActions();
        ...
        else
        {
               ...
               (bSeen || bHeard))
            {
                if(GetIsDead(oPerceived))
                {
                    ...
                    SpeakString("Percieved Dead Friend! Moving into investigate", TALKVOLUME_SHOUT);
                ...
                else
                {
                    ...
                    SpeakString("Percieved Alive Fighting Friend! Moving to and attacking.", TALKVOLUME_SHOUT);
