...
//
//   EDITED BY: Ronan
//      -kept spawns from executing NESS behaviors when posessed.
//      -kept added behavor on/off flag for DM tools.
//
   ...
   int nTimeNow, int nWaypoint)
{
    ...
    for (nChildSlot = 1; nChildSlot <= nSpawnNumber; nChildSlot++)
    {
        ...
        if (nSpawnChild == TRUE)
        {
            ...
            // Behavior master flag
            int behaviorDisabled = GetLocalInt(oCreature, "behDis");

            // Random Walk
            if (!behaviorDisabled && !GetIsDMPossessed(oCreature) && nRandomWalk == TRUE && nDespawning == FALSE && nSpawnDespawn == FALSE)
            {
            ...
            // Patrol
            if (!behaviorDisabled && !GetIsDMPossessed(oCreature) && nPatrolRoute > -1 && nDespawning == FALSE && nSpawnDespawn == FALSE)
            {
            ...
            // ReturnHome
            if (!behaviorDisabled && !GetIsDMPossessed(oCreature) && nReturnHome == TRUE && nDespawning == FALSE && nSpawnDespawn == FALSE)
            {
            ...
            // Spawn Sit
            if (!behaviorDisabled && !GetIsDMPossessed(oCreature) && nSpawnSit == TRUE && nDespawning == FALSE)
            {
            ...
            else  // is not placeable, camp, item, or merchant
            {
                   ...
                   || (nPCCheck == TRUE && nPCCount == 0))
                {
                    ...
                    if (nEntranceExit > -1)
                    {
                        ...
                        AssignCommand(oCreature, ActionDoCommand(CleanInventory(oCreature)));
                        if (nSaveState)
                        {
...
{
    vector vSpawnPos;
    // lSpawnLocation is where we spawn in; it may get set to
    // lEntranceExit
    location lSpawnLocation, lEntranceExit;
    ...
    if (nSpawnEffect > 0)
    {
        ...
        if (nEntranceExit > -1)
        {
            // This is a bug; lEntranceExit hasn't been found yet...
            ApplyEffectAtLocation(DURATION_TYPE_TEMPORARY, eSpawn, lEntranceExit,
               5.0);
        }
        else
        {
    ...
    if (sTemplate != "")
    {
        ...
        if (nSpawnCamp == TRUE)
        {
            ...
            RecordSpawned(oSpawn, oSpawned, lSpawnLocation, lSpawnLocation, fSpawnFacing);
        }
        else
        {
            // EntranceExit
            ...
            RecordSpawned(oSpawn, oSpawned, lSpawnLocation, lSpawnLocation,
               fSpawnFacing);
            SetupSpawned(oSpawn, oSpawned, lSpawnLocation, nTimeNow,
               nWalkToHome);
        }
    }
}
