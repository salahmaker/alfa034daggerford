...
// -------------------------------------------
    ...
    else if (nLootTable == 2)
    {
        ...
        int nItemChance = Random(20) + 1;

        if (nItemChance > 98 )
        {
             oItem = CreateItemOnObject("duergaraxe003", oSpawned);

        }

        if (nItemChance > 96 && nItemChance < 99  )
        {
             oItem = CreateItemOnObject("duergarshield002", oSpawned);
        }

        if (nItemChance > 94 && nItemChance < 97  )
        {
             oItem = CreateItemOnObject("duergarhelm002", oSpawned);
        }
        if (nItemChance > 91 && nItemChance < 95  )
        {
             oItem = CreateItemOnObject("duergaraxe002", oSpawned);
        }

         if (nItemChance > 80 && nItemChance < 92  )
        {
             oItem = CreateItemOnObject("duergaraxe01", oSpawned);
        }

        if (nItemChance > 70 && nItemChance < 81  )
        {
             oItem = CreateItemOnObject("duergarhelm01", oSpawned);
        }

        if (nItemChance > 60 && nItemChance < 71  )
        {
             oItem = CreateItemOnObject("duergarshield01", oSpawned);
        }
        ...
        {
             oItem = CreateItemOnObject("ironoresmall002", oSpawned);
        }
        if (nItemChance > 40 && nItemChance < 51  )
        {
             oItem = CreateItemOnObject("ironoresmall001", oSpawned);
        }

        if (nItemChance < 39  )
        {
             oItem = CreateItemOnObject("ironoresmall", oSpawned);
        }
    }
    // Deurger chest loot table
    ...
    // Borderlands Keep Skeleton loot table
    ...
    // Borderlands Keep skelteon chest loot table
