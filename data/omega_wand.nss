...
void main()
{
 ...
 if(sItem == "omega_wand")
    ...
    // Set the original target location.
    SetLocalLocation(oOwner, "OMOrigLocation", lLoc);

    // Set effect duration if it has not been set before.
    if(GetLocalFloat(oOwner, "DURATION_TIME") == 0.0f) {
        SetLocalFloat(oOwner, "DURATION_TIME", 300.0);
        SetCustomToken( 6971, "5 minutes." );
    }
