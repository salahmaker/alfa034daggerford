...
// * improves a natural attack of any target, cycling through creature weapons and arms slot
int ApplyIPToItemInSlot(object oCreature, itemproperty ip, int nInventorySlot, float nDuration);
void DoAltMagicFang(object oTarget, int nPower, float nDuration);
// * Backwards compatibility to old DoMagicFang
...
//:://////////////////////////////////////////////
/*
    Checks target to see if they have
    a spell absorption effect on them
    ie. Lesser or Greater Spell Mantle
*/
//:://////////////////////////////////////////////
//:: Created By: Shawn (U'lias) Marcil
//:: Created On: August 28, 2003
//:://////////////////////////////////////////////
int HasSpellMantle(object oTarget)
{
    int bHasSpellMantle = FALSE;
    effect eCurrentEffect = GetFirstEffect(oTarget);
    // cycle through all the targets effects looking for a spell level
    // absorption effect
    while (GetIsEffectValid(eCurrentEffect))
    {
        if (GetEffectType(eCurrentEffect) == EFFECT_TYPE_SPELLLEVELABSORPTION)
        {
            bHasSpellMantle = TRUE;
            break;
        }
        eCurrentEffect = GetNextEffect(oTarget);
    }

    return bHasSpellMantle;
}
...
//::
//:: Modified August 27 2003 - by Shawn (U'lias) Marcil for ALFA
/*
    -Added check for spell type immunity.
    -Added Reflex Save for half damage.
    -Added check for a Lesser or Greater Spell Mantle so that no
        damage is done to a target beyond the first missle to account
        for the spell mantle's absorption of spell level only once in
        the case of multiple missles.
    -Re-wrote missle targetting to cycle through the same enemies
        so that all extra missles don't just hit the first target
        but are spread out amongst all enemies in the vicinity.
*/
...
/**** BEGIN ALFA CODE ****/
        ...
        /* ALFA Algorithm
            ...
            3. All excess missiles get divided
                equally among targets as best it can
       */
    int nEnemies = 0;
    ...
    while (GetIsObjectValid(oTarget) )
    {
        ...
        // ALFA:    altered parameter SPELL_TARGET_SELECTIVEHOSTILE to
        //          SPELL_TARGET_STANDARDHOSTILE to address PvP concerns
    ...
    switch (nSpell)
    {
        case SPELL_ISAACS_GREATER_MISSILE_STORM: nSpellLevel = 6;
            nSaveType = SAVING_THROW_TYPE_SPELL;
            break;
        case SPELL_FIREBRAND: nSpellLevel = 5;
            nSaveType = SAVING_THROW_TYPE_FIRE;
            break;
        default: nSpellLevel = 4; // SPELL_ISAACS_LESSER_MISSILE_STORM
            nSaveType = SAVING_THROW_TYPE_SPELL;
            break;
    }

    int nAbilMod = GetCasterAbilityModifier(oCaster);
    int nDC = 10 + nSpellLevel + nAbilMod;

    int m = 0;
    int nTimes = 1;

    // loop for the number of missles (potentially 1 enemy can be the recipient of all missles)
    for (m=1; m <= nMissiles; m++)
    {
        // Firebrand.
        // It means that once the target has taken damage this round from the
        // spell it won't take subsequent damage
        if (    (nTimes == 2) && (nONEHIT == TRUE)    ) return;
        ...
        while (    (GetIsObjectValid(oTarget)) && (nCnt <= nMissiles)    )
        {
                    ...
                    (oTarget != oCaster)    )
            {
                ...
                // get whether or not target has spell absorption in effect
                int bHasSpellMantle = HasSpellMantle(oTarget);

         /*       if (DEBUG_MODE == TRUE)
                {
                    SendMessageToPC(oTarget, "bHasSpellMantle " + IntToString(bHasSpellMantle));
                    SendMessageToPC(oTarget, "nTimes " + IntToString(nTimes));
                }     */

                /*
                    Check if this is the first time the target has been missled or
                    check if the previously missled target doesn't have a spell mantle.
                    Unfortunately, if this spell reduces a Spell Mantles spell level
                    absorption to "0" and missles beyond the first one will
                    hit the target.
                */
                if (    (nTimes == 1) ||
                        (    (nTimes > 1) && (bHasSpellMantle != TRUE)    ) )
                {
                    // Make SR Check
                    if (!MyResistSpell(oCaster, oTarget, fDelay))
                    {
                        ...
                        nDam = GetReflexAdjustedDamage(nDam, oTarget, GetSpellSaveDC(), nSaveType);
                    ...
                    /*        if (DEBUG_MODE == TRUE)
                            {
                                sMessage = "*Spell Avoidance*";
                                FloatingTextStringOnCreature(sMessage, oTarget, TRUE);
                            }    */
                        }
                    }
                    else
        ...
        if (nCnt > nMissiles) break; // if the count of missles fired exceeds the number cap of missles exit
        nTimes++; // increment the number of loops to find enemy targets before missles run out
...
//:://////////////////////////////////////////////
/*
 Enhancement bonus based on calling function - +1
 for Magic Fang, +1 per 4 caster levels for GMF.

 Applies to creature weapons first as temporary
 enhancement, like GMW.  If not creature weapons,
 tries item slot (separate attack and damage).  If
 not, fails casting.
*/
//:://////////////////////////////////////////////
//:: Created By:
//:: Created On:
//:: Modified On: July 28, 2006 (Ronan/ATD)
//:://////////////////////////////////////////////

int ApplyIPToItemInSlot(object oCreature, itemproperty ip, int nInventorySlot, float nDuration) {
    object oItem = GetItemInSlot(nInventorySlot, oCreature);
    if(GetIsObjectValid(oItem)) {
        IPSafeAddItemProperty(oItem, ip, nDuration);
        return 1;}
        else {
        return 0; }
}
...
void DoAltMagicFang(object oTarget, int nPower, float nDuration) {
    ...
    itemproperty ip = ItemPropertyEnhancementBonus(nPower);
    // Apply bonus to creature weapon slots first.  If any are valid, return
    if( ApplyIPToItemInSlot(oTarget, ip, INVENTORY_SLOT_CWEAPON_B, nDuration) ||
        ApplyIPToItemInSlot(oTarget, ip, INVENTORY_SLOT_CWEAPON_L, nDuration) ||
        ApplyIPToItemInSlot(oTarget, ip, INVENTORY_SLOT_CWEAPON_R, nDuration)) {
    // Add VFX to indicate successful cast, steal from GMW
        ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDur, oTarget, nDuration);
        return;        }

    // If no creature weapons, check to see if gloves applicable, as with monk gloves
    // Requires Attack and Damage separate instead of Enhancement due to slot
    itemproperty ipAttack = ItemPropertyAttackBonus(nPower);
    itemproperty ipDamage = ItemPropertyDamageBonus(DAMAGE_TYPE_BLUDGEONING, nPower);
    // Try applying to arms, if not, indicate no valid creature items can be enhanced
    if( !ApplyIPToItemInSlot(oTarget, ipAttack, INVENTORY_SLOT_ARMS,nDuration) ) {
        string sMessage = "The target of this spell must have a creature weapon or gloves equiped.";
        SendMessageToPC(oTarget, sMessage);
        SendMessageToPC(OBJECT_SELF, sMessage);
        return;
    }
    ApplyIPToItemInSlot(oTarget, ipDamage, INVENTORY_SLOT_ARMS, nDuration);
    // Add VFX to indicate successful cast
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDur, oTarget, nDuration);
}
